...
GigECDataProcess::GigECDataProcess(wqueue<imgFrame*>&queue,LPVOID *lpUser)
	:cpm_queue(queue)
{
	m_hThread=NULL;
	//m_pPutMsg=NULL;
...
int GigECDataProcess::SetProcType( DataProcessType type )
{
	CGuard guard(m_Mutex);
	m_ProcType=type;
	return 0;
...
int GigECDataProcess::SetChangeType( RgbChangeType type )
{
	CGuard guard(m_Mutex);
	m_ChangeType=type;
	return 0;
