...
GigECDataCapture::GigECDataCapture(wqueue<imgFrame*>&queue, GigECDataProcess *dp)
	:m_queue(queue)
{
	m_pDataProcess=NULL;
	m_iCount=0;		
		...
		dataCnt += PACKSIZE;
			if(nRet>0)
			{
				...
				if((camNum!=camNum_last)&&(p_udpbuf[0]==0x33))//boarder pack check,new frame came
				{
					...
					lostpacks=packnum-1;
					if((lostpacks>0)&&(lostpacks<MAX_RESEND_SIZE))//new frame's head lost
					{
				...
				else if(p_udpbuf[0]==0x33&&packnum!=0)
				{
					...
					if((lostpacks>0)&&(lostpacks<MAX_RESEND_SIZE))
					{
						resendbuf[0]=0x56;
						resendbuf[1]=0xab;
						resendbuf[2]=0x03;//resend order
...
int GigECDataCapture::getProp(clientPropStruct* prop)
{
	int buffsize=32;
...
int GigECDataCapture::sendProp(clientPropStruct prop)
{
...
int GigECDataCapture::sendOrder(camPropStruct camprop,int s)
{
	char *sendbuff=new char[32];
