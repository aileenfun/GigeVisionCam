...
int clientProp::setClientProp(clientPropStruct prop)
{
	memcpy(&memClientProp,&prop,sizeof(clientPropStruct));
	initBuffer();
	return 0;
}
int clientProp::getClientProp(clientPropStruct *prop)
{
	memcpy(prop,&memClientProp,sizeof(clientPropStruct));
	return 0;
}
int clientProp::setpcIP(DWORD IP)
{
	memClientProp.pcIP=IP;
	return 0;
}
unsigned long clientProp::getpcIP()
{
	return memClientProp.pcIP;
}
int clientProp::setcamIP(DWORD IP)
{
	memClientProp.camIP=IP;
	return 0;
}
unsigned long clientProp::getcamIP()
{
	return memClientProp.camIP;
}
int clientProp::getBuffer(char * destbuffer)
{
	memcpy(destbuffer,sendbuffer,32);
	return 32;
}
int clientProp::setCamCnt(int cnt)
{
	memClientProp.camCnt=cnt;
	return cnt;
}
int clientProp::getCamCnt()
{
